...
- 其他选项（域名、NTP服务器等）`,
  ...
  },
  {
    id: "p2p-model",
    title: "P2P应用模型",
    brief: "P2P网络的基本概念，与C/S模型的对比，DHT结构化P2P",
    keyTakeaways: [
      "P2P中每个节点既是客户端又是服务器，资源分散在所有参与者中",
      "P2P的可扩展性优于C/S：节点越多，总资源（带宽、存储）越多",
      "非结构化P2P：无固定拓扑，查询靠泛洪或随机游走（如Gnutella）",
      "结构化P2P（DHT）：用哈希函数确定资源位置，查询效率O(logN)（如Chord）",
      "BitTorrent是混合模型：用Tracker或DHT找到peer，peer之间P2P传输",
    ],
    commonMistakes: [
      "认为P2P完全不需要服务器（实际上很多P2P系统有辅助服务器如Tracker）",
      "混淆结构化和非结构化P2P的查询方式",
      "认为P2P只用于文件共享（实际还有VoIP、区块链、CDN等应用）",
    ],
    sections: [
      {
        id: "p2p-vs-cs",
...
**C/S模型**：
- 服务器上传带宽 us，每个客户端下载带宽 di
- 分发时间 ≥ max(NF/us, F/dmin)
- 随N线性增长（服务器瓶颈）

**P2P模型**：
- 分发时间 ≥ max(F/us, F/dmin, NF/(us + Σui))
- 随N增长缓慢（每个新节点贡献上传带宽）

### 关键优势

P2P的总上传能力随节点数增加而增加，具有**自扩展性**。
...
节点之间的连接关系没有特定规则，形成随机的覆盖网络。

### 查询方式
- **泛洪（Flooding）**：向所有邻居广播查询，邻居继续转发（设TTL限制范围）
- **随机游走（Random Walk）**：查询随机转发给一个邻居
- 缺点：查询效率低，无法保证找到资源

### 代表系统
- Gnutella：纯P2P，完全去中心化
- Napster：混合式，中央索引服务器+P2P传输

## 结构化P2P（DHT）

**DHT（Distributed Hash Table，分布式哈希表）**：用一致性哈希将资源和节点映射到同一个标识空间。

### 基本思想

1. 每个节点和每个资源都有一个m位的标识符（ID）
2. 资源存储在ID最接近的节点上
3. 查询时，通过路由表逐步接近目标ID

### Chord环（经典DHT）

把所有节点和资源想象成坐在一个圆形餐桌上，每个座位有编号（0到2^m-1）。规则很简单：**每份资源由它顺时针方向遇到的第一个节点保管**。

\`\`\`
节点和资源映射到 [0, 2^m-1] 的环上（m=6，范围0~63）：
...
资源key=10 → 顺时针第一个节点是N14 → N14负责保管
资源key=3  → 顺时针第一个节点是N8  → N8负责保管
资源key=55 → 顺时针第一个节点是N56 → N56负责保管
\`\`\`

### Finger Table（路由加速表）

如果每个节点只知道下一个邻居，查询就要一步步绕环走，最坏O(N)跳。Chord的巧妙之处在于每个节点维护一张**finger table**——记录距离自己 2^0, 2^1, 2^2, ..., 2^(m-1) 位置处的后继节点：

\`\`\`
N8的finger table（m=6）：
finger[0]: N8+1=9   → 后继N14
finger[1]: N8+2=10  → 后继N14
finger[2]: N8+4=12  → 后继N14
finger[3]: N8+8=16  → 后继N42
finger[4]: N8+16=24 → 后继N42
finger[5]: N8+32=40 → 后继N42
\`\`\`

查询时，节点选择finger table中**最接近但不超过目标**的节点转发，每一跳至少跨越剩余距离的一半，因此：
- 查询复杂度：**O(logN)跳**
- 每个节点存储：**O(logN)个指针**
- 节点加入/离开：只需更新相邻节点和finger table，影响范围有限

## BitTorrent

混合架构的P2P文件共享系统，核心思想是"人人为我，我为人人"：

- **Tracker**：记录哪些peer拥有文件的哪些块（类似公告板）
- **种子（Seed）**：拥有完整文件的peer（无私奉献者）
- **下载者（Leecher）**：正在下载的peer（边下边传）
- **分块传输**：文件分成固定大小的块（通常256KB），每块独立下载

### 两个关键策略

**稀缺优先（Rarest First）**：优先下载网络中副本最少的块。想象一本书被撕成100页分给100个人，如果第42页只有一个人有，他一旦离开这页就永远凑不齐了。所以大家都优先复印第42页，确保稀缺页尽快扩散。

**以上传换下载（Tit-for-Tat）**：你给我传得快，我也优先给你传。这是一种激励机制——不劳动者不得食。具体做法：每个peer只向上传速度最快的4个邻居提供服务（unchoke），每30秒随机解锁一个新邻居（optimistic unchoke）给新人机会。白嫖者（只下不传）会被所有人"掐断"。`,
      },
    ],
    exercises: [
      {
        id: "p2p-ex1",
        title: "P2P模型分析",
        description: "回答以下问题：\n(1) P2P模型相比C/S模型在文件分发场景下的主要优势是什么？\n(2) 结构化P2P（DHT）相比非结构化P2P的优势是什么？代价是什么？\n(3) BitTorrent中「稀缺优先」策略的目的是什么？",
        difficulty: "medium" as const,
        hints: [
          "考虑节点数增加时的带宽变化",
          "非结构化P2P的查询用泛洪",
          "如果某个块只有一个peer拥有，该peer离开会怎样",
        ],
        referenceSolution: `(1) P2P的主要优势是自扩展性。在C/S模型中，服务器带宽固定，N个客户端的分发时间随N线性增长。在P2P中，每个新加入的节点不仅消耗带宽，也贡献上传带宽，总上传能力随节点数增加而增加，分发时间增长缓慢。

(2) 结构化P2P（DHT）的优势：查询效率高，O(logN)跳即可定位资源，且能保证找到存在的资源（确定性查询）。非结构化P2P用泛洪查询，效率低且不能保证找到。代价：需要维护路由表（finger table），节点加入/离开时需要更新路由信息，维护开销较大。

(3) 稀缺优先策略的目的是提高文件的可用性。优先下载网络中副本最少的块，使得稀缺块尽快被复制到更多节点上。如果不这样做，某个只有少数peer拥有的块可能因为这些peer离开而永久丢失，导致所有下载者都无法获得完整文件。`,
      },
    ],
    relatedLessons: ["dns-http"],
  },
