...
(3) TLB结构（全相联）：
  ...
  {
    id: "flash-multimodule",
    title: "Flash存储器与多模块存储器",
    brief: "Flash存储器的类型与特点，多体并行存储器的交叉编址技术",
    keyTakeaways: [
      "Flash分NOR（随机读、适合代码存储）和NAND（页读写、适合大容量数据存储）",
      "Flash写前必须擦除，擦除次数有限，需要磨损均衡",
      "高位交叉编址：同一模块连续编址，适合扩容，不能并行",
      "低位交叉编址：相邻地址分布在不同模块，支持流水线式并行访问",
      "m个模块低位交叉，连续访问m个字的时间 = T + (m-1)×τ（T为存取周期，τ为总线传输周期）",
    ],
    analogy: "低位交叉编址像超市多个收银台：顾客（地址）轮流分配到不同收银台（模块），前一个还在结账时后一个已经开始扫码，流水线式提高吞吐量。",
    commonMistakes: [
      "混淆高位交叉和低位交叉的编址方式",
      "忘记Flash写前必须擦除这一特性",
      "计算低位交叉访问时间时用错公式",
...
Flash（闪存）是一种电可擦除的非易失性存储器，介于EPROM和EEPROM之间。
...
|------|-----------|------------|
| 读取方式 | 随机读（类似RAM） | 页读取 |
| 写入方式 | 字节/字写入 | 页写入（通常4KB） |
| 擦除单位 | 块（64KB~128KB） | 块（128KB~256KB） |
| 读取速度 | 快（可XIP执行） | 较慢 |
| 写入速度 | 慢 | 快 |
| 容量 | 较小（MB级） | 大（GB~TB级） |
| 成本 | 高 | 低 |
| 典型应用 | BIOS、嵌入式代码 | SSD、U盘、SD卡 |

### Flash的关键特性

1. **写前擦除**：Flash不能直接覆盖写，必须先擦除整个块再写入
2. **擦除次数有限**：NOR约10万次，NAND约1万~10万次
3. **磨损均衡（Wear Leveling）**：通过FTL（Flash Translation Layer）将写操作均匀分布到各块，延长寿命
4. **坏块管理**：标记和跳过损坏的块

### SSD的基本结构

\`\`\`
主机接口(SATA/NVMe) → SSD控制器 → Flash芯片阵列
                         ↓
                    FTL(闪存转换层)
                    - 地址映射
                    - 磨损均衡
...
\`\`\``,
      ...
      {
        id: "multimodule-memory",
        title: "多模块存储器",
        type: "detail",
        content: `## 为什么需要多模块存储器

单个存储模块的存取周期 T 远大于CPU时钟周期。如果CPU连续访问同一模块，必须等待上一次存取完成——就像只有一个窗口的银行，每个客户办理业务要5分钟，后面的人只能干等。

解决方案：开多个窗口（模块），让多个客户同时办理。

## 高位交叉编址（顺序编址）

地址的**高位**选择模块号，低位为模块内地址。
...
相当于把银行的号码牌按楼层分：1楼办1~100号，2楼办101~200号。如果你连续取号1、2、3，全在1楼排队——**不能并行**。

- 优点：便于存储器扩容（加一个模块就多一段连续地址）
- 用途：主要用于扩大存储容量

## 低位交叉编址（交叉编址）

地址的**低位**选择模块号，高位为模块内地址。
...
相当于4个窗口轮流叫号：窗口0叫1号，窗口1叫2号，窗口2叫3号，窗口3叫4号。连续的客户分散到不同窗口，**流水线式并行**。

## 低位交叉访问时间计算

设：
- m = 模块数
- T = 单模块存取周期（一个模块从接收地址到数据就绪的时间）
- τ = 总线传输周期（数据从模块送到CPU的时间）
- 要求：**T ≤ m × τ**（保证流水不阻塞）

为什么有这个条件？因为第1个模块开始工作后，经过 τ 时间启动第2个模块，再过 τ 启动第3个……第m个模块启动时经过了 (m-1)×τ 时间。当第m个模块的数据准备好时，第1个模块必须已经完成（耗时T），否则就要等待。所以需要 T ≤ m×τ。
...
理解：第1个字等完整的T才出来，之后每隔 τ 就有一个字从不同模块流出（像流水线出产品）。
...
4个模块，T=200ns，τ=50ns：
- 验证：T=200 ≤ m×τ=4×50=200 ✓（恰好满足）
- 顺序访问4个字：4 × 200 = 800ns
- 交叉访问4个字：200 + 3×50 = 350ns
- 加速比：800/350 ≈ 2.3倍`,
      },
    ],
    exercises: [
      {
        id: "flash-multi-ex1",
        title: "多模块存储器计算",
        description: "某计算机采用4体低位交叉存储器，每个模块的存取周期为80ns，总线传输周期为20ns。\n(1) 该设计是否满足流水线不阻塞的条件？\n(2) 连续读取8个字需要多少时间？\n(3) 与单模块相比加速比是多少？",
        difficulty: "medium",
        hints: [
          "不阻塞条件：T ≤ m × τ",
          "连续访问n个字（n>m）：T + (n-1)×τ",
        ...
        referenceSolution: `(1) T=80ns, m=4, τ=20ns
    m×τ = 4×20 = 80ns = T ✓ 恰好满足不阻塞条件

(2) 连续读取8个字：
    时间 = T + (n-1)×τ = 80 + 7×20 = 80 + 140 = 220ns

(3) 单模块：8 × 80 = 640ns
    加速比 = 640 / 220 ≈ 2.9倍`,
      },
    ],
    relatedLessons: ["memory-hierarchy"],
  },
];
